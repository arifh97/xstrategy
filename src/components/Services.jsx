...
function Services(){
    ...
    return (
      <section className='services shape section-padding overflow-hidden position-relative'>
        <div className="services-shape position-absolute"><img src={serviecShape} alt="" /></div>
        <div className="services-logo position-absolute"><img src={logoBig} alt="" /></div>
        <Container>
          <Row>
            <Col lg="5" className='position-sticky'>
              <div className="services-timeline bg-primary w-100 rounded-1">
                <ul className='d-flex align-items-center'>
                  <li className='flex-grow-0'><span className='fw-bold'>01</span></li>
                  <li className='flex-grow-0 w-100'><ProgressBar now={30}></ProgressBar></li>
                  <li className='flex-grow-0'><span className='fw-bold'>03</span></li>
                  <li className='flex-grow-0'><span className='fw-bold'>04</span></li>
                </ul>
              </div>
              <div className='whatWeDo-content pt-2 pt-md ps-0 pb-5 pb-lg-0'>
                <span className="meta_text text-uppercase text-primary d-inline-block mb-3">digital entertainment</span>
                <h3 className='text-white-2 mb-4'>Web3 Gaming and Digital Entertainment Brands</h3>
                <p className="mb-3">Through white-label or bespoke custodial or non-custodial wallet solutions, Web3 Gaming companies can access an extensive.</p>
                <p>XSTRATEGY has partnered with best-in-class, blockchain native technology stacks that make-up a secure.</p>
                <Button variant='primary' className='mt-4 mt-lg-5'>
                  Explore More <FontAwesomeIcon icon={faInfoCircle} />
                </Button>
              </div>
            </Col>
            <Col lg={{ span: 6, offset: 1 }} className="d-md-flex flex-wrap d-lg-block gap-md-4 gap-lg-0">
              {servicesData.map((item, index) => (
                <div className="services-card" key={index}>
                  ...
                  <p>{item.text}</p>
                  <a href="#" className='d-flex align-items-center gap-2'>Learn More <FontAwesomeIcon icon={faArrowRight} /></a>
                </div>
              ))}
            </Col>
          </Row>
